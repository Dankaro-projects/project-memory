...
Update `pyproject.toml`, `memory_module.__version__`, the Codex plugin version, `server.json`, the release commands and changelog together. Python uses `0.5.0b3`; plugin metadata uses its SemVer form `0.5.0-beta.3`. The CLI constructs the versioned GitHub wheel URL from the Python version.
...
python scripts/publish_smithery.py dist/project-memory-0.5.0-beta.3.mcpb --name YOUR_NAMESPACE/project-memory
```
