...
After release, use the published command in a fresh project and perform actual capture and recovery. Keep raw logs and operational reports private. Public documentation should contain maintained reproduction instructions and clearly bounded claims, not installation diaries or session-specific results. Never replace an already published wheel with different code. Fixes receive a new beta version.
...
Run Build, inspect the actual handshake and `tools/list` result, then create a release using the matching SemVer beta version. Auto-Release is disabled because this configuration pins a specific commit; update the pin and test each release before publishing. This container supports provider inspection; it does not connect Glama to a user's local database. Follow the README for normal local installation.
