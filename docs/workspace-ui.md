...
- The overview shows up to three work items per displayed state, four recent decisions and three proposed lessons. Counts and links expose the full lists. It reads existing records without generating summaries or inferring approval. The live overview uses one bounded response; deriving work states still examines the project work records.
